...
Lab 已完成 NAS Constellation 与动态前端运行框架。原 Temporal Field 经内容评审后撤销正式项目身份：它证明了 WebGL、Canvas、低动态偏好和资源释放能力，但缺少对博客的实际用途，因此只保留为未公开的视觉能力原型。

Visual Systems 的首个正式内容重新定义为 `VS–001 Archive Atlas／档案地图`。它必须以真实博客内容探索为核心；具体设计见 `VS001_ARCHIVE_ATLAS.md`。用户后续提供的动态实验可作为 VS–002，也可在更合适时调整编号。
...
Lab
├── Visual Systems
│   ├── Archive Atlas（VS–001，真实 Archive 索引）
│   └── Temporal Field renderer（未公开视觉原型，不是项目）
└── NAS Observatory
    └── NAS Constellation
```
...
## 4. Visual Systems 首个正式项目

### 4.1 内容决策

Temporal Field 不再是公开 Lab 项目，也不占用 `VS–001`。原因不是渲染技术不足，而是它作为独立页面没有帮助访问者理解、发现或使用博客内容。

其 Shader、Canvas fallback、暂停、reduced-motion 与资源清理代码可以继续作为视觉能力原型，但只有在服务真实内容、转场或交互反馈时才允许重新使用。视觉修饰关闭后，页面的核心信息和功能必须仍然成立。

### 4.2 VS–001 Archive Atlas

Archive Atlas 是博客的可视化内容入口：以发布时间、内容类型、阅读长度、状态和显式关联映射公开 Archive Objects，让访问者可以搜索、筛选、选择并进入文章。

它不是关系图谱炫技，也不根据模糊文本相似度虚构关系。所有关联必须来自公开、明确的 `related` 字段；完整数据映射、交互、技术方案和验收条件见 `VS001_ARCHIVE_ATLAS.md`。

### 4.3 当前状态

- Visual Systems 板块继续保留。
- `VS–001 Archive Atlas` 已接入七个真实公开 Archive Objects，并标记为正式可用；模型记忆不得绕过 MDX 内容审核进入公开索引。
- Temporal Field 页面与公开 manifest 入口撤下，源组件暂时作为未发布原型保存。
- VS–002 保留给用户提供的动态实验，不强制使用 VS–001 模板；已确认的工程来源、技术边界与验收基线见 `VS002_TECHNICAL_BASELINE.md`。
...
### Phase 2：视觉运行时原型（已完成并撤出公开项目）

- 已验证 WebGL、Canvas、资源隔离和生命周期能力。
- 原型仅作为视觉修饰能力保存，不再独立发布。

### Phase 2B：Archive Atlas（真实内容接入已完成）

- 从公开 Archive Objects 生成最小内容索引。
- 完成搜索、筛选、时间空间、详情和文章入口。
- 以真实内容价值和可访问性作为首要验收条件。
...
- Lab 首页只展示真实可访问项目。
- Temporal Field 不再出现在公开路由、入口或 manifest。
- Archive Atlas 已完成真实资料替换；后续以构建期 MDX 索引和内容核查规则持续更新。
...
- Temporal Field 的渲染代码保留为未公开视觉原型，不构成内容项目或发布要求。
- Archive Atlas 的内容索引、映射和交互以 `VS001_ARCHIVE_ATLAS.md` 为实现基线。
